...
public class PremierLeagueDAO {
	...
	public List<Team> vertici() {
		String sql = "  SELECT TeamID, name,m.ResultOfTeamHome "
				+ " FROM teams t, matches m "
				+ " WHERE m.TeamHomeID=t.TeamID "
				+ " ORDER BY TeamID";
		...
		try {
			...
			while (res.next()) {
				if (res.isFirst()) {
					Team team = new Team(res.getInt("TeamID"), res.getString("Name"), new ArrayList<Integer>());
					team.getEsiti().add(res.getInt("m.ResultOfTeamHome"));
					result.add(team);
				} else {
					if (result.get(result.size() - 1).getTeamID() == res.getInt("TeamID")) {
						result.get(result.size() - 1).getEsiti().add(res.getInt("m.ResultOfTeamHome"));
